...
////////////////////////////////////////////////////////////////////////////
    ...
    if((in = fopen("encrypted.enc", "r")) == NULL){
        fprintf(stderr, "Problems read the new file\n");
        abort();
    }

    if((encrypted_len = fread(encrypted_msg, 1, RSA_size(rsa_key_pair), in)) != RSA_size(rsa_key_pair))
        handle_errors();

    fclose(in);

    unsigned char decrypted_msg[RSA_size(rsa_key_pair)];

    if(RSA_private_decrypt(encrypted_len, encrypted_msg, decrypted_msg, rsa_key_pair, RSA_PKCS1_OAEP_PADDING) == -1)
        handle_errors();
