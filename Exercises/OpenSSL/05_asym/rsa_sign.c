...
// argv[2] is the name of the file in which the private key is stored
...
int main(int argc, char **argv){
    ...
    FILE *f_key;
    if((f_key = fopen(argv[1], "r")) == NULL){
        fprintf(stderr, "Couldn't open the input file. Try again\n");
        exit(1);
    }

    //DIGEST SIGN --> EVP_PKEY * 
    ...
    if(!EVP_DigestSignFinal(sign_ctx, NULL, &dig_len))
        handle_errors();
    ...
    if(fwrite(signature, 1, sig_len, out) < sig_len){
        fprintf(stderr, "Couldn't save data. Try again\n");
        exit(1);
    }
